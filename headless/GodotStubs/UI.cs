// Audited from sts2-cli/src/GodotStubs/UI.cs.
// Status: clean — but two important semantic decisions are encoded here:
//   • Tween.Play() / .CustomStep() / .Stop() fires `Finished` immediately.
//   • Timer.Start() fires `Timeout` immediately.
//   • AnimationPlayer.Play() now also fires `AnimationFinished` (sts2-cli's
//     stub didn't, which would deadlock anything `await`-ing the signal via
//     a bare event handler. ToSignal() pattern is unaffected since that
//     uses SignalAwaiter; we add the event fire for direct `+=` users.)
//
// The Tween/Timer/AnimationPlayer instant-completion is the core "make
// headless go fast" mechanism: any game code that schedules visual delay
// gets immediate completion instead. This is correct iff sts2's logic
// doesn't depend on intermediate tween *progress* — only on whether the
// tween has finished. So far validated for combat, rewards, map, event,
// rest-site, treasure, shop. Watch this space if a new room phase blows up.
...
public class Control : CanvasItem
{
    ...
    public new Rect2 GetViewportRect() => new Rect2(Vector2.Zero, StubDisplay.ViewportSize);
...
public class Timer : Node
{
    ...
    // Risk-flagged: fires Timeout synchronously on Start. This drops the
    // wall-clock delay, which is the whole point in headless. Game code
    // that uses await ToSignal(timer, "timeout") goes through SignalAwaiter
    // and is unaffected; code that uses `+= handler; Start()` works because
    // Timeout fires after Start returns.
...
public class AnimationPlayer : Node
{
    ...
    // Diff vs sts2-cli: their Play() was a no-op, which would deadlock any
    // direct subscriber. We fire AnimationFinished synchronously so the
    // animation is "instantly done." `await ToSignal(player, "animation_finished")`
    // routes through SignalAwaiter (always completed) and is unaffected
    // either way; this fix is for raw `+= handler` users.
...
public class GpuParticles2D : Node2D
{
    ...
    public bool OneShot { get; set; }
    public bool Explosiveness { get; set; }
}
